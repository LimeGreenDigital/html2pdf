...
<html>
    ...
    protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
    {
        ...
        if ($inTL[0] ?? -1 <=0 || $inTL[1]<=0) $inTL = null;
        if ($inTR[0] ?? -1 <=0 || $inTR[1]<=0) $inTR = null;
        if ($inBR[0] ?? -1 <=0 || $inBR[1]<=0) $inBR = null;
        if ($inBL[0] ?? -1 <=0 || $inBL[1]<=0) $inBL = null;
    ...
    protected function _tag_open_PAGE($param)
    {
        ...
        if (array_key_exists('hideheader', $param) && $param['hideheader']!='false' && !empty($param['hideheader'])) {
            $this->_hideHeader = array_merge($this->_hideHeader, explode(',', $param['hideheader']));
        }
